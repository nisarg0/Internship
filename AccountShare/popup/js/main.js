...
(function () {
  ...
  const template = {
    ...
    get: function (name) {
      ...
      if (!templateEl)
        throw new Error("Whoops! template " + id + " does not exist");
    ...
    "attach-share": function () {
      ...
      addEventListener(
        ...
        function updateExpiresText(event) {
          ...
          if (timeout.trim()) {
            expiresOn.innerText = `Expires on: ${expires.getExpirationString(
              timeout
            )}`;
      ...
      addEventListener("#js-share-session", "submit", function () {
        try {
          ...
          session.store(
            publicKey,
            expirationTime,
            function (encryptedData, tab) {
              show("js-shared-session");

              getElementById(
                "js-shared-session-text"
              ).innerHTML = encryptedData;
          ...
          console.warn(e);
          showError(
            "An error occurred trying to encrypt your session. Check that the code is correct."
          );
        }
      });

      addEventListener("#js-session-hide", "click", () =>
        hide("js-shared-session")
      );

      addEventListener("#js-copy-share-text", "click", () =>
        hide("js-shared-session")
      );
      ...
      function displayLink(encryptedData) {
        shareText.getLink(
          ...
          function error() {
            getElementById("js-share-text-link").innerHTML =
              "Whops, couldn't get the link";
          }
        );
      }
    },
    ...
    "attach-restore": function () {
      ...
      addEventListener("#js-restore-session", "submit", function (event) {
        try {
          ...
          console.warn(e);
          showError(
            "An error occurred trying to restore the session. Check that the encrypted text is correct and was generated with your code."
          );
        }
      });
      ...
      this.onTextSubmitted('[name="encrypted-data"]', function (textarea) {
        ...
        try {
          ...
          if (expires.isExpired(expirationTime))
  ...
  const expires = {
    ...
    getExpirationTime(timeoutInHours) {
      timeoutInHours = this.isValidTimeout(timeoutInHours)
        ? timeoutInHours
        : this.DEFAULT;
  ...
  function addEventListener(selector, event, fn) {
    ...
    if (!els.length)
      log("[WARN] Could not find an element for selector " + selector);
